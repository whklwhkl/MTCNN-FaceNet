...
def get_face_without_background(f_image):
    from PIL import Image
    image = Image.open(f_image)
    bindingBoxes = get_bindingBoxes(image)
    f_image=f_image.split('\\')[1]
    name,ext=f_image.split('.')
    if not os.path.exists(name):
        os.makedirs(name)
    image.crop(bindingBoxes[0,:-1]).save(name+'\\1.'+ext)
...
if __name__ == "__main__":
    ...
    for f in os.listdir(folder):
        try:
            get_face_without_background(os.path.join(folder,f))
        except:
            print(f)
